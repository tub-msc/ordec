...
small descriptor referencing its blob. All columns of a typical
SimHierarchy view into one shared buffer, so the table degenerates to a
single entry. Blob bytes participate in the hash, so wire identity is
...
from .simarray import SimColumn, SimSeries, Quantity
...
def encode_plain(val, blobs: BlobTable=None):
    ...
    if t is SimSeries:
        return cbor2.CBORTag(TAG_SIMSERIES,
            [encode_simcolumn(val.values, blobs),
                [encode_simcolumn(s, blobs) for s in val.scales]])
...
def fix_plain(val, blobs):
    ...
    if isinstance(val, RefMarker):
        ...
        if val.tag == TAG_SIMSERIES:
            if not (isinstance(val.value, (list, tuple))
                    and len(val.value) == 2):
                raise WireError("Malformed SimSeries encoding.")
            values, scales = val.value
            try:
                return SimSeries(fix_plain(values, blobs),
                    [fix_plain(s, blobs) for s in scales])
            except (TypeError, ValueError) as e:
                raise WireError(f"Malformed SimSeries encoding: {e}") from e
...
def resolve_row_value(val, deps, ept, blobs):
    if isinstance(val, RefMarker) and val.tag not in (TAG_SIMCOLUMN,
            TAG_SIMSERIES):
