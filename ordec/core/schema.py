    ...
    def portmap(self, **kwargs):
        def inserter_func(main, sgu):
            ...
            for k, v in kwargs.items():
                SchemInstanceConn(
                    ref=main_nid, here=v.nid, there=self[k].nid
                ).insert_into(sgu)
            return main_nid
...
class SchemInstanceConn(Node):
    ...
    here = LocalRef(Net)
    there = ExternalRef(
        Pin, of_subgraph=lambda c: c.ref.symbol
    )  # ExternalRef to Pin in SchemInstance.symbol
...
class SimInstance(NonLeafNode):
    ...
    schematic = SubgraphRef(
        Symbol | Schematic,
        typecheck_custom=lambda v: isinstance(v, (Symbol, Schematic)),
    )
    eref = ExternalRef(
        SchemInstance, of_subgraph=lambda c: parent_siminstance(c.parent).schematic
    )
