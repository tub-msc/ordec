...
#. **Flatten** (``_flatten``) — the schematic is expanded recursively to Metal1-only
...
* After every net is routed, each **conflict** raises the cost of the offending grid
  ...
  nets, **or** two nets too close given the 210 nm wires + 150 nm end extensions — Metal3
...
* **Min area (0.144 µm²) and the via endcap (50 nm)** cannot be met by an isolated via
  ...
  a post-pass (``_extend_min_area``) lengthens any too-short segment into free tracks to
  reach min area.
...
* **Scale** — it targets blocks of tens of cells, where production tools handle millions.
  ...
  annealing, and routing is split into global routing (congestion estimation) + detailed
  routing instead of a single flat maze.
