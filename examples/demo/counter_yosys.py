...
def main():
    lib = Path(os.getenv('ORDEC_PDK_IHP_SG13G2')) / 'libs.ref/sg13g2_stdcell/lib/sg13g2_stdcell_typ_1p20V_25C.lib'
    verilog = synthesize([Path(__file__).parent/'counter.v'], 'counter', lib)
    print(verilog)
    data = read_verilog(verilog)
