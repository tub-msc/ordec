...
def test_op_currents(sim_batch):
    tb = lib_test.ResdivHierTb()
    h = tb.sim_dc_batch if sim_batch else tb.sim_dc_piped
...
def test_generic_mos_nmos_sourcefollower(sim_batch):
    view = 'sim_dc_batch' if sim_batch else 'sim_dc_piped'
    assert getattr(lib_test.NmosSourceFollowerTb(vin=R(2)), view).o.voltage[0] == pytest.approx(1.2837721914145377, abs=1e-6)
    assert getattr(lib_test.NmosSourceFollowerTb(vin=R(3)), view).o.voltage[0] == pytest.approx(2.2837721567191442, abs=1e-6)

def test_op_save_params(sim_batch):
    tb = lib_test.NmosSourceFollowerTb(vin=R(2))
    h = tb.sim_dc_params_batch if sim_batch else tb.sim_dc_params_piped
    nmos = h.I0
    assert nmos.params['gm'].value[0] > 0
    assert nmos.params['gds'].value[0] == pytest.approx(0)

def test_generic_mos_inv(sim_batch):
    view = 'sim_dc_batch' if sim_batch else 'sim_dc_piped'
    assert getattr(lib_test.InvTb(vin=R(0)), view).o.voltage[0] == pytest.approx(4.999999973230277, abs=1e-6)
    assert getattr(lib_test.InvTb(vin=R('2.5')), view).o.voltage[0] == pytest.approx(2.499999990010493, abs=1e-6)
    assert getattr(lib_test.InvTb(vin=R(5)), view).o.voltage[0] == pytest.approx(0, abs=1e-6)
...
def test_generic_mos_inv_dc_sweep(sim_batch):
    ...
    h = tb.sim_dc_sweep_batch if sim_batch else tb.sim_dc_sweep_piped
...
def test_sky_mos_inv(sim_batch):
    view = 'sim_dc_batch' if sim_batch else 'sim_dc_piped'
    assert getattr(lib_test.InvSkyTb(vin=R(0)), view).o.voltage[0] == pytest.approx(4.999999973187308, abs=1e-10)
    assert getattr(lib_test.InvSkyTb(vin=R('2.5')), view).o.voltage[0] == pytest.approx(1.9806063550640076, abs=1e-10)
    assert getattr(lib_test.InvSkyTb(vin=R(5)), view).o.voltage[0] == pytest.approx(0.00012158997833462999, abs=1e-10)

def test_ihp_mos_inv_vin0(sim_batch):
    view = 'sim_dc_batch' if sim_batch else 'sim_dc_piped'
    h_0 = getattr(lib_test.InvIhpTb(vin=R(0)), view)
    assert h_0.o.voltage[0] == pytest.approx(4.999573)

def test_ihp_mos_inv_vin5(sim_batch):
    view = 'sim_dc_batch' if sim_batch else 'sim_dc_piped'
    h_5 = getattr(lib_test.InvIhpTb(vin=R(5)), view)
...
def test_webdata():
    # Test DC webdata
    h_dc = lib_test.ResdivFlatTb().sim_dc_batch
    sim_type, data = h_dc.webdata()
    assert sim_type == 'report'
    assert 'elements' in data

    # Test transient webdata
    sim_type, data = lib_test.PulsedRC().sim_tran_batch.webdata()
    assert sim_type == 'report'
    assert 'elements' in data

    # Test DC sweep webdata
    sim_type, data = lib_test.InvTb().sim_dc_sweep_batch.webdata()
    assert sim_type == 'report'
    assert 'elements' in data
