...
def test_op_save_params():
    ...
    arr = h.to_numpy(include=[nmos.params['gm']])
    assert arr.dtype.names == ("I0.params['gm'].value",)
    assert arr["I0.params['gm'].value"][0] == nmos.params['gm'].value[0]

def test_op_save_params_nested():
    """Devices below the top level need their device type letter prefixed.

    The inverter's mosfets sit inside the i_inv subcircuit. Without the
    "m." prefix ngspice accepts the save directive but reports zeros for
    every parameter, so check the name here: both devices are biased off,
    which makes the values themselves legitimately ~0.
    """
    tb = lib_test.InvTb()
    h = tb.sim_op
    assert h.i_inv.pd.params['gm'].value.name == "@m.xi_inv.mpd[gm]"
...
def test_ihp130_inv_op_save_params():
    """Device parameters of mosfets wrapped in a PSP103 model subcircuit.

    These are only reachable as @n.<path>.<inner device>[param]; addressing
    the subcircuit call instead yields zeros for every parameter. vth is
    checked because it holds regardless of bias (both devices are off).
    """
    tb = lib_test.InvIhpTb()
    h = tb.sim_op
    assert h.i_inv.pd.params['vth'].value.name == \
    ...
    assert h.i_inv.pu.params['vth'].value[0] == pytest.approx(0.4599, abs=1e-3)

def test_ihp130_inv_dc_sweep():
    tb = lib_test.InvIhpTb()
    h = tb.sim_dc
    assert h.sim_type == SimType.DCSWEEP
    # vin=0 -> output ~5V
